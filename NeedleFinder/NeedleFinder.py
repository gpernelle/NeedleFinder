...
matArcLen_mm = [
    ...
    [46.675, 46.675, 46.675, 46.675, 46.675, 46.675, 46.675, 46.675],
    [
        56.00999999999999,
        56.00999999999999,
        56.00999999999999,
    ...
    [65.345, 65.345, 65.345, 65.345, 65.345, 65.345, 65.345, 65.345],
    [
        74.67999999999999,
        74.67999999999999,
        74.67999999999999,
    ...
    [93.35, 93.35, 93.35, 93.35, 93.35, 93.35, 93.35, 93.35],
    [
        102.68499999999999,
        102.68499999999999,
        102.68499999999999,
    ...
    [130.69, 130.69, 130.69, 130.69, 130.69, 130.69, 130.69, 130.69],
    [
        140.02499999999998,
        140.02499999999998,
        140.02499999999998,
    ...
    [158.695, 158.695, 158.695, 158.695, 158.695, 158.695, 158.695, 158.695],
    [
        168.02999999999997,
        168.02999999999997,
        168.02999999999997,
...
global matEndSegAngles_rad
matEndSegAngles_rad = [
    [
        0.0,
        0.0048911031291655645,
        0.008772931009455694,
        0.012654758889745826,
        0.016536586770035954,
        0.020418414650326087,
        0.024300242530616217,
        0.03206389829119648,
    ],
    [
        0.0,
        0.0097821477537871314,
        0.017545524419748475,
        0.025308504507977683,
        0.033070912554819608,
        0.040832573110985634,
        0.048593310745960024,
        0.064111315650571224,
    ],
    [
        0.0,
        0.014672958367230593,
        0.026316767563298378,
        0.03795819785140836,
        0.049596197489461008,
        0.06122971568278901,
        0.072857702874107963,
        0.096092893944324409,
    ],
    [
        0.0,
        0.019563242504854809,
        0.035084973522322038,
        0.050598779293131932,
        0.066101162284007281,
        0.081588634958592948,
        0.097057722817595948,
        0.12792692936615674,
    ],
    [
        0.0,
        0.0244525908664339,
        0.043847783399815349,
        0.063223182748141038,
        0.082570082109233611,
        0.10187983015169963,
        0.12114384767997007,
        0.15950083308144927,
    ],
    [
        0.0,
        0.029340477558769017,
        0.052602170814584542,
        0.075822363657736896,
        0.098982892771041053,
        0.12206581017065535,
        0.14505344663060321,
        0.19067395934661271,
    ],
    [
        0.0,
        0.034226260500106548,
        0.061344449377726845,
        0.088385345375096638,
        0.11531536587396353,
        0.142101590650139,
        0.16871195132043398,
        0.22128212659299801,
    ...
    [
        0.0,
        0.039109182027560384,
        0.070070283883116619,
        0.10089928821551089,
        0.13153936763625315,
        0.16193542142574976,
        0.19203454935196959,
    ...
    [
        0.0,
        0.043988369748074949,
        0.078774705905577169,
        0.11334958494160875,
        0.147623212913919,
        0.18150976791276655,
        0.21492843045803769,
    ...
    [
        0.0,
        0.048862837672509679,
        0.087452134444779228,
        0.1257199857599105,
        0.16353212172384282,
        0.20076255248713809,
        0.23729556138390023,
...
matYDefl_mm = [
    ...
    [
        -0.0,
        0.045658265663975975,
        0.081894260474511926,
        0.11812902125575206,
        0.15436200200171157,
        0.19059265673322845,
        0.22682043950619019,
...
MAXNEEDLES = (
    MAXCOL
) = 206  # we have no more than 206 distinct colors defines here for display
...
class NeedleFinderLogic:
    ...
    def __init__(self):
        ...
        self.tableValueCtrPt = [
            [[999, 999, 999] for i in range(100)] for j in range(100)
        ]
    ...
    def loadCTLPointsInTable(self):
        ...
        nodes = slicer.mrmlScene.GetNodesByClassByName(
            "vtkMRMLAnnotationHierarchyNode", "Fiducials List"
        )
        addPointBool = 0
        ...
        print("*" * 50)
        print(
            "%d list(s) of fiducials found with the same name - add point bool: %d"
            % (int(nodes.GetNumberOfItems()), addPointBool)
        )
        print("*" * 50)
    ...
    def undoFid(self):
        ...
        widget = slicer.modules.NeedleFinderWidget
        print(
            "..Undoing fiducial point mvt for pt ", widget.undoListFid[-1].get("name")
        )
    ...
    def takeScreenshot(self, name, description, type=-1, annotate=True):
        ...
        if annotate == True:
            ...
            annotationLogic.CreateSnapShot(
                name, description, type, self.screenshotScaleFactor, imageData
            )
    ...
    def hideIsoSurfaces(self):
        ...
        widget = slicer.modules.NeedleFinderWidget
        if contourNode != None:
            contourNode.SetDisplayVisibility(
                abs(widget.hideContourButton.isChecked() - 1)
            )
            contourNode.GetModelDisplayNode().SetSliceIntersectionVisibility(
                abs(widget.hideContourButton.isChecked() - 1)
            )
    ...
    def AddContour(self, polyData):
        ...
        displayNode.SetAndObserveColorNodeID(
            "vtkMRMLColorTableNodeFileHotToColdRainbow2.txt"
        )
    ...
    def array2(self):
        ...
        shape.reverse()
        a = vtk.util.numpy_support.vtk_to_numpy(i.GetPointData().GetScalars()).reshape(
            shape
        )
        labels = []
        val = [[0, 0, 0] for i in range(a.max() + 1)]
        for i in range(2, a.max() + 1):
    ...
    def deleteDuplicateMarkers(self):
        ...
        profprint()
        scene = slicer.mrmlScene
        nodes = scene.GetNodesByClassByName(
            "vtkMRMLAnnotationHierarchyNode", "Fiducials List"
        )
        if nodes.GetNumberOfItems() > 1:
            for i in range(1, nodes.GetNumberOfItems()):
                ...
                if hnode:
                    ...
                    hnode.GetAssociatedChildrenNodes(
                        children, "vtkMRMLAnnotationFiducialNode"
                    )

                    for i in range(children.GetNumberOfItems()):
                        annotNode = children.GetItemAsObject(i)

                        oneToOneHierarchyNode = hnode.GetAssociatedHierarchyNode(
                            annotNode.GetScene(), annotNode.GetID()
                        )
        ...
        nodes = slicer.mrmlScene.GetNodesByClassByName(
            "vtkMRMLAnnotationHierarchyNode", "All Annotations"
        )
        if nodes.GetNumberOfItems() > 1:
            for i in range(1, nodes.GetNumberOfItems()):
                try:
    ...
    def placeNeedleShaftEvalMarker(
        ...
        if (
            ...
            displayNode = fiducial.GetDisplayNode()
            displayNode.SetGlyphScale(2)
            displayNode.SetColor(
                self.color[int(nth)][0],
                self.color[int(nth)][1],
                self.color[int(nth)][2],
            )
            textNode = fiducial.GetAnnotationTextDisplayNode()
            textNode.SetTextScale(4)
            textNode.SetColor(
        ...
        self.addPointToTable(
            pointName, needleNumber, stepValue, None, sortTable=createPoint
        )
    ...
    def obturatorNeedle(self, A, imageData, colorVar, spacing):
        ...
        * For the following needles, only give the tip of the needle. It will draw parallel to the first obturator needle
        ...
        displayNode.SetGlyphScale(2)
        displayNode.SetColor(
            self.color[int(nth)][0], self.color[int(nth)][1], self.color[int(nth)][2]
        )
        textNode = fiducial.GetAnnotationTextDisplayNode()
        textNode.SetTextScale(4)
        textNode.SetColor(
    ...
    def objectiveFunction(
        ...
        radiusNeedleCorner = int(
            round((radiusNeedleParameter / float(spacing[0]) / 1.414))
        )
        ...
        if gradientPonderation != 0:
            ...
            g2 = imageData.GetScalarComponentAsDouble(
                ijk[0] - radiusNeedle, ijk[1], ijk[2], 0
            )
            g3 = imageData.GetScalarComponentAsDouble(
                ijk[0], ijk[1] + radiusNeedle, ijk[2], 0
            )
            g4 = imageData.GetScalarComponentAsDouble(
                ijk[0], ijk[1] - radiusNeedle, ijk[2], 0
            )
            g5 = imageData.GetScalarComponentAsDouble(
                ijk[0] + radiusNeedleCorner, ijk[1] + radiusNeedleCorner, ijk[2], 0
            )
            g6 = imageData.GetScalarComponentAsDouble(
                ijk[0] - radiusNeedleCorner, ijk[1] - radiusNeedleCorner, ijk[2], 0
            )
            g7 = imageData.GetScalarComponentAsDouble(
                ijk[0] - radiusNeedleCorner, ijk[1] + radiusNeedleCorner, ijk[2], 0
            )
            g8 = imageData.GetScalarComponentAsDouble(
                ijk[0] + radiusNeedleCorner, ijk[1] - radiusNeedleCorner, ijk[2], 0
            )
        ...
        for I in range(-X, X):
            ...
            for J in range(-Y, Y):
                ...
                for k in range(1):
                    ...
                    totalTip = 0
                    for l in range(-3, 1):
                        v0 = 8 * imageData.GetScalarComponentAsDouble(
                            A[0] + i, A[1] + j, A[2] + k + l, 0
                        )

                        v1 = imageData.GetScalarComponentAsDouble(
                            A[0] + radiusNeedle + i, A[1] + j, A[2] + k + l, 0
                        )
                        v2 = imageData.GetScalarComponentAsDouble(
                            A[0] - radiusNeedle + i, A[1] + j, A[2] + k + l, 0
                        )
                        v3 = imageData.GetScalarComponentAsDouble(
                            A[0] + i, A[1] + radiusNeedle + j, A[2] + k + l, 0
                        )
                        v4 = imageData.GetScalarComponentAsDouble(
                            A[0] + i, A[1] - radiusNeedle + j, A[2] + k + l, 0
                        )
    ...
    def findAxialSegmentationLimitFromMarker(self, bForceFallback=False):
        ...
        for node in list(nodes.values()):
            ...
            if aslNew[2] > asl[2]:
                ...
                if found:
                    print(
                        "higher limit marker found in scene, z-limit [ras]: ", coord[2]
                    )
                else:
                    print(
                        "first limit marker found in scene, z-limit [ras]: ", coord[2]
                    )
            if found:
    ...
    def interp3(self, V, x, y, z, interpswitch):
        ...
        if interpswitch == 1:
            ...
            value = V.GetScalarComponentAsDouble(
                int(round(x)), int(round(y)), int(round(z)), 0
            )
        return value
    ...
    def needleDetectionAPI(
        ...
        if widget.algoVersParameter.value in range(0, 4):
            ...
            for NeedleIndex in range(len(tips)):
                ...
                if widget.algoVersParameter.value == 0:
                    self.needleDetectionThreadCurrentDev(
                        A,
                        imageData,
                        colorVar,
                        spacing,
                    ...
                    self.needleDetectionThread13_1(
                        A,
                        imageData,
                        colorVar,
                        spacing,
                    ...
                    self.needleDetectionThread13_2(
                        A,
                        imageData,
                        colorVar,
                        spacing,
                    ...
                    self.needleDetectionThread15_1(
                        A,
                        imageData,
                        labelData,
        ...
        if not widget.algoVersParameter.value in range(0, 6):
            msgbox(
                "/!\ needleDetectionThread %d not defined"
    ...
    def needleDetectionThread(
        ...
        widget = slicer.modules.NeedleFinderWidget
        (
            widget.axialSegmentationLimit,
            widget.axialSegmentationLimitRAS,
        ) = self.findAxialSegmentationLimitFromMarker()
        if widget.labelMapNode:
            labelData = widget.labelMapNode.GetImageData()
        else:
            labelData = None
    ...
    def needleDetectionThread_RM(
        ...
        global conesColor
        if conesColor:
            conesColor = (conesColor + 1) % 308
        if conesColor == 0:
            conesColor = 300
        print("conesColor= ", conesColor)
        ### initialisation of some parameters
        fModelNeedleLength_mm = 187
        fModelSegmentLength_mm = fModelNeedleLength_mm / 20.0
        fK = 2 * np.pi * 2050 / (1000.0)  # 18 gauge brachy needle
        ...
        for NeedleIndex in range(NumberOfNeedles):
            ...
            if widget.algoVersParameter.value == 1:
                lvControlPointsRAS, lvControlPointsIJK = self.needleDetectionThread13_1(
                    ijkA,
                    imgData,
                    colorVar,
                    fvSpacing,
                ...
                lvControlPointsRAS, lvControlPointsIJK = self.needleDetectionThread15_1(
                    ijkA,
                    imgData,
                    imgLabelData,
                    lrasTempPoints,
                    ...
                    bUp=False,
                    bScript=False,
                    strManualName=strManualName,
                    tipOnly=False,
                    bDrawNeedle=False,
                )
            else:
                msgbox(
                    "/!\ single needleDetectionThread %d not defined"
                    % widget.algoVersParameter.value
                )
        ...
        for feedbackindex in range(NumberOfFeedbacks):
            if feedbackindex == 0:
                (
                    ...
                    WrongReason,
                    GlobalDirection,
                ) = self.needleRationalityCheck(
                    ControlPointsPackageIJK, ControlPointsPackage, imgData, fvSpacing
                )
                ...
                for i in range(NumberOfWrongPositionsToSend):
                    ...
                    if widget.algoVersParameter.value == 1:
                        ...
                        (
                            ...
                            strManualName,
                            bDrawNeedle=False,
                            whetherfeedback=True,
                            ControlPointsPackage=ControlPointsPackage,
                            ControlPointsPackageIJK=ControlPointsPackageIJK,
                            WrongPosition=WrongPositionsToSend[i],
                ...
                if NumberOfWrongNeedles == 0:
                    ...
                    for i in range(NumberOfWrongPositionsToSend):
                        ...
                        print(
                            "The ",
                            WrongPositionsToSend[i],
                            "(+1 each) Needle corrected",
                        )
                    LastWrongPositionsToSend = WrongPositionsToSend

        (
            FinalControlPointsPackage,
            FinalControlPointsPackageIJK,
        ) = self.needleRationalityProtect(
            OriginalControlPointsPackage,
            OriginalControlPointsPackageIJK,
            ControlPointsPackage,
            ControlPointsPackageIJK,
        ...
        for i in range(NumberOfNeedles):
            ...
            if not bAutoStopTip:
                self.addNeedleToScene(
                    FinalControlPointsPackage[i],
                    (205) % MAXCOL,
                    "Detection",
                    bScript,
                    0,
                    manualName=strManualName,
                )
    ...
    def needleRationalityProtect(
        ...
        for i in range(NumberOfNeedles):
            ...
            direction = [
                ControlPointsPackage[i][0][0]
                - ControlPointsPackage[i][NumberOfPointsOnThisNeedle - 1][0],
                ControlPointsPackage[i][0][1]
                - ControlPointsPackage[i][NumberOfPointsOnThisNeedle - 1][1],
                ControlPointsPackage[i][0][2]
                - ControlPointsPackage[i][NumberOfPointsOnThisNeedle - 1][2],
            ]
        ...
        GlobalDirection = [
            GlobalDirection[0] / NumberOfNeedles,
            GlobalDirection[1] / NumberOfNeedles,
            GlobalDirection[2] / NumberOfNeedles,
        ]
        for i in range(NumberOfNeedles):
            tempmetric1 = 1 - (
        ...
        for i in range(NumberOfNeedles):
            ...
            if NumberOfPointsOnThisNeedle > 2:
                for j in range(NumberOfPointsOnThisNeedle - 2):
                    direction1 = [
                        ControlPointsPackage[i][j][0]
                        - ControlPointsPackage[i][j + 1][0],
                        ControlPointsPackage[i][j][1]
                        - ControlPointsPackage[i][j + 1][1],
                        ControlPointsPackage[i][j][2]
        ...
        for i in range(NumberOfNeedles):
            NumberOfPointsOnThisNeedle = len(ControlPointsPackage[i])
            currentDistances = [
                ([0] * NumberOfNeedles) for k in range(NumberOfPointsOnThisNeedle)
            ]
        ...
        DistanceBetweenEachOther = [
            ([0] * NumberOfNeedles) for k in range(NumberOfNeedles)
        ]
        ...
        NearestCoordinate = [
            ([([0] * 3) for k in range(NumberOfNeedles)])
            for n in range(NumberOfNeedles)
        ]
        ...
        for i in range(NumberOfNeedles):
            ...
            for j in range(NumberOfNeedles):
                ...
                print(
                    " (%d" % NearestPoint[i][j],
                    ")%.2f" % DistanceBetweenEachOther[i][j],
                    end=" ",
                )
            print(" ")
        ...
        for i in range(NumberOfNeedles):
            if i < 9:
                ...
                print("Needle ", i + 1, ":", end=" ")
            for j in range(NumberOfNeedles):
                print(" %d" % OverlapCounter[i][j], end=" ")
            print(" ")
        print("Check End")
            ...
            for j in range(i + 1, NumberOfNeedles):
                if OverlapCounter[i][j] > 0 or OverlapCounter[j][i] > 0:
                    ...
                    if joinflag1 == 1:
                        ...
                        direction1 = [
                            ControlPointsPackage[i][0][0] - NearestCoordinate[i][j][0],
                            ControlPointsPackage[i][0][1] - NearestCoordinate[i][j][1],
                            ControlPointsPackage[i][0][2] - NearestCoordinate[i][j][2],
                        ]
                        direction2 = [
                            ControlPointsPackage[j][0][0] - NearestCoordinate[i][j][0],
                            ControlPointsPackage[j][0][1] - NearestCoordinate[i][j][1],
        ...
        for i in range(NumberOfOriginalWrongPositions):
            if (
                ...
                FinalControlPointsPackage[
                    OriginalWrongPositions[i][0]
                ] = OriginalControlPointsPackage[OriginalWrongPositions[i][0]]
                FinalControlPointsPackageIJK[
                    OriginalWrongPositions[i][0]
                ] = OriginalControlPointsPackageIJK[OriginalWrongPositions[i][0]]
            # else:
      ...
      """
        ...
        for step in range(0, NbStepsNeedle + 1):
            ...
            if step == 0:
                ...
                tIter = max(
                    1, int(round(stepSize))
                )  # ## ??? stepSize can be smaller 1 and it is in mm not int index coordinates
        ...
        """
                ...
                stepSize = self.stepSize(step + 1, NbStepsNeedle + 1) * lenghtNeedle
                rMax = max(
                    stepSize, distanceMax / float(spacing[0])
                )  # ??? why is stepSize not divided by spacing[0]
                rIter = max(
                    15, min(20, int(rMax / float(spacing[0])))
                )  # ??? why divide rMax again by spacing[0]
                tIter = max(
                    1, int(round(stepSize))
                )  # ## ??? stepSize can be smaller 1 and it is in mm not int index coordinates
            ...
            P0 = A
            C0 = [P0[0] + K * Vx, P0[1] + K * Vy, P0[2] + K * Vz]
                    ...
                    for t in range(tIter + 1):
                        ...
                        if (
                            ...
                            if gradient == 1 and mode == "circle":
                                ...
                                g5 = imageData.GetScalarComponentAsDouble(
                                    ijk[0] + radiusNeedleCorner,
                                    ijk[1] + radiusNeedleCorner,
                                    ijk[2],
                                    0,
                                )
                                g6 = imageData.GetScalarComponentAsDouble(
                                ...
                                total += ((center - g1) ** gradientPonderation) / float(
                                    tIter
                                )
                                total += ((center - g2) ** gradientPonderation) / float(
                                    tIter
                                )
                                total += ((center - g3) ** gradientPonderation) / float(
                                    tIter
                                )
                                total += ((center - g4) ** gradientPonderation) / float(
                                    tIter
                                )
                                total += ((center - g5) ** gradientPonderation) / float(
                                    tIter
                                )
                                total += ((center - g6) ** gradientPonderation) / float(
                                    tIter
                                )
                                total += ((center - g7) ** gradientPonderation) / float(
                                    tIter
                                )
                                total += ((center - g8) ** gradientPonderation) / float(
                                    tIter
                                )
                            ...
                            # >>>>>>>>>>>>>>>>>>>>>> exp.02
                            if imgLabelData:
                                fLabel = imgLabelData.GetScalarComponentAsFloat(
                                    ijk[0], ijk[1], ijk[2], 0
                                )
                                if fLabel and fLabel < 300:
                                    ...
                                    total -= 10000
                                if conesColor:
                                    imgLabelData.SetScalarComponentFromFloat(
                                        ijk[0], ijk[1], ijk[2], 0, conesColor
                                    )  # mark the search cones in fLabel volume
                            # <<<<<<<<<<<<<<<<<<<<
                    if R == 0:
    ...
    def needleDetectionThread13_1(
        ...
        for step in range(0, NbStepsNeedle + 2):
            ...
            if step == 0:
                ...
                tIter = int(
                    round(L)
                )  # ??? L can be smaller 1 and it is in mm not int index coordinates
            ...
            # ------------------------------------------------------------------------------
                ...
                C0 = [
                    ...
                    2 * A[1] - tip0[1],
                    A[2] - stepSize,
                ]  # ??? this is buggy vector calculus, now its a feature ;-)

                rMax = max(stepSize, distanceMax / float(spacing[0]))
                rIter = max(
                    15, min(20, int(rMax / float(spacing[0])))
                )  # ??? why divide again by spacing[0]
            ...
            for R in range(int(rIter) + 1):
                ...
                # ## angle variation from 0 to 360
                for thetaStep in range(int(nbRotatingStep)):
                    ...
                    for t in range(int(tIter) + 1):
                        ...
                        if (
                            ...
                            # >>>>>>>>>>>>>>> Ruibins feedback >>>>>>>>>>>>>>>>>
                            if whetherfeedback:
                                NeighbourAttenuation = self.Feedback(
                                    ControlPointsPackage,
                                    Punish,
                                    radiusNeedleParameter,
                            ...
                            if gradient == 1:
                                ...
                                radiusNeedle = int(
                                    round(radiusNeedleParameter / float(spacing[0]))
                                )
                                radiusNeedleCorner = int(
                                    round(
                                        (
        ...
        if not autoStopTip:
            if bDrawNeedle:
                self.addNeedleToScene(
                    controlPoints, colorVar, "Detection", script, 0, manualName=manName
                )
    ...
    def needleDetectionThread13_2(
        ...
        for step in range(0, NbStepsNeedle + 2):
            ...
            if step == 0:
                ...
                tIter = int(
                    round(L)
                )  ## /spacing[2] worse at beginnng! more steps??? L can be smaller 1 and it is in mm not int index coordinates
            ...
            # ------------------------------------------------------------------------------
                ...
                C0 = [
                    2 * A[0] - tip0[0],
                    2 * A[1] - tip0[1],
                    A[2] - stepSize / spacing[2],
                ]  # <<< /spacing[2] bugfix # ??? this is buggy vector calculus, now its a feature ;-)
        ...
        if not autoStopTip:
            self.addNeedleToScene(
                controlPoints, colorVar, "Detection", script, manualName=manualName
            )
    ...
    """
        ...
        def defLocalNeedleCooSys(
            ...
            if not rasNeedlePlaneNormal.any():
                rasNeedlePlaneNormal = np.cross(rasSegmentVector1, fvZ)
            if np.arccos(np.dot(rasNeedlePlaneNormal, fvX)) > np.pi / 2:
                rasNeedlePlaneNormal *= (
                    -1
                )  # check vector direction and turn if not in rhs normal X direction
            ...
            if not rasDeflectionDirection.any():
                rasDeflectionDirection = +np.cross(
                    fvZ, rasNeedlePlaneNormal1
                )  # TODO / CAVE: mind the sign
            if np.arccos(np.dot(rasDeflectionDirection, fvY)) > np.pi / 2:
                rasDeflectionDirection *= -1  # check vector direction and turn for RHS
        ...
        def initNeedleModel(
            ...
            faYDefl_mm = matYDefl_mm[
                ixArcLenColMin
            ]  # zip(*matYDefl_mm)[ixArcLenColMin] #transpose
        ...
        def runNeedleModel(
            ...
            if 1 and fDistBDef2BMod > fMaxDistBDef2BMod_mm:
                ...
                rasBDef2BMod = rasB - rasBDef
                rasBDef2BMod1, fDistBDef2BMod_mm = normalized(rasBDef2BMod)
                rasB = rasBDef + rasBDef2BMod1 * min(
                    fMaxDistBDef2BMod_mm, fDistBDef2BMod_mm / 2.0
                )
            fAngleMultiSegments_rad = 0
            if fStepSize_mm / fModelSegmentLength_mm < 1:
                iFineStep = 5
                print("# fine steps ")
            else:
                iFineStep = 1
            ...
            for j in range(
                ...
                angle_deg = np.degrees(fStepAngle_rad)
                fSumAngle_rad -= fStepAngle_rad
                fSumAngle_deg = np.degrees(fSumAngle_rad)
                if fSumAngle_rad < 0:
                    fStepAngle_rad = fSumAngle_rad = 0
                    # TODO check this
        ...
        if not bScript:
            widget.createAddOrSelectLabelMapNode(
                bScript
            )  # <<< always have label map to draw search cones
        # widget.clearLabelMap()

        if iAxialSegmentationLimit != None:
            fEstNeedleLength_mm = abs(
                ijkA[2] - iAxialSegmentationLimit
            )  # * 1.15 * fvSpacing[2] #<<< CONST was 1.15
        else:
        ...
        fAngleRefNeedle_rad = np.deg2rad(
            22.5
        )  # <<< to rotate z-axis down 22.5 degrees around x
        fvX = np.array([1, 0, 0])
        fvY = -np.array([0, np.cos(fAngleRefNeedle_rad), np.sin(fAngleRefNeedle_rad)])
        ...
        if not bUp:
            self.controlPoints = (
                []
            )  # in up mode, more interpol. points will be added to this array
        lvControlPointsRAS = []
        lvControlPointsIJK = []
        lvBestControlPoints = []
        ...
        for iAbsStep, iStep in enumerate(range(iStart, nStepsNeedle)):
            ...
            if iStep > -1:
                fStepsSum_mm += fStepSize_mm
            print(
                "fStepSize_mm, fStepsSum_mm, fEstNeedleLength_mm= %f, %f, %f"
                % (fStepSize_mm, fStepsSum_mm, fEstNeedleLength_mm)
            )
            ...
            if iStep < 0:
                ...
                iRMax = max(
                    fStepSizeInit_mm, iRadiusMax_mm / float(fvSpacing[0])
                )  # / nStepsNeedle
            ...
            # ------------------------------------------------------------------------------
                ...
                ijkB = [
                    ijkA[0],
                    ijkA[1],
                    ijkA[2] + iZDirectionSign * fStepSize_mm / fvSpacing[2],
                ]  # <<< going down along z-axis performs better on average on MICCAI13 cases
                rasB = self.ijk2ras(ijkB)
                ijkEstStepVector = np.array(
                    [0, 0, iZDirectionSign * fStepSize_mm / fvSpacing[2]]
                )

                if len(rasA2CBestInit) == 0:
                    rasA2CBestInit.append(fvZ)
                ...
                if iStart < 0 and iStep == 0:
                    fF_mmN, fSumAngle_rad = initNeedleModel(
                        fAngleRefNeedle_rad, fEstNeedleLength_mm, fvZ, rasA
                    )
            ...
            # ------------------------------------------------------------------------------
                ...
                ijkB = [
                    2 * ijkA[0]
                    - ijkAPrev[0],  # ??? why do you go double iStep in xy-plane
                    2 * ijkA[1] - ijkAPrev[1],
                    ijkA[2] + iZDirectionSign * fStepSize_mm / fvSpacing[2],
                ]  # ??? this was buggy vector calculus, now its a feature ;-)
                ...
                nRIter = max(15, min(20, int(iRMax / fvSpacing[0])))
                nTIter = (
                    fStepSize_mm  # ??? coarse step size from MICCAI13 TODO check this
                )
                print("iRMax, nRIter, nTIter= %d, %d, %d" % (iRMax, nRIter, nTIter))
                ...
                (
                    rasDeflectionDirection1,
                    rasNeedlePlaneNormal1,
                    fvZ,
                ) = defLocalNeedleCooSys(
                    fvX,
                    fvY,
                    fvZ,
                    iStep,
                ...
                if iStart == 0 and iStep == 1:
                    fF_mmN, fSumAngle_rad = initNeedleModel(
                        fAngleRefNeedle_rad, fEstNeedleLength_mm, fvZ, rasA, bDraw=True
                    )
                # run needle model
            ...
            for iR in range(int(nRIter) + 1):
                ...
                for iThetaStep in range(int(nRotatingIts)):

                    fAngle_deg = (iThetaStep * 360.0) / float(nRotatingIts)
                    fThetaStep_rad = math.radians(fAngle_deg)

                    ijkC = [
                        ijkB[0] + fR * (math.cos(fThetaStep_rad)),
                        ijkB[1] + fR * (math.sin(fThetaStep_rad)),
                        ijkB[2],
                    ]

                    fTotal = 0
                    lijkM = [[0, 0, 0] for i in range(int(nTIter) + 1)]
                    ...
                    for iTStep in range(int(nTIter) + 1):
                        ...
                        # first, test if points are in the image space
                        iRadiusNeedle = int(
                            round(iRadiusNeedle_mm / float(fvSpacing[0]))
                        )
                        iRadiusNeedleCorner = int(
                            round((iRadiusNeedle_mm / float(fvSpacing[0]) / 1.414))
                        )
                        ...
                        if (
                            ...
                            if 1 and bGradient == 1:  # <<< feature on/off
                                ...
                                iRadiusNeedleCorner = int(
                                    round(
                                        (iRadiusNeedle_mm / float(fvSpacing[0]) / 1.414)
                                    )
                                )

                                g1 = imgData.GetScalarComponentAsDouble(
                                    ijk[0] + iRadiusNeedle, ijk[1], ijk[2], 0
                                )
                                g2 = imgData.GetScalarComponentAsDouble(
                                    ijk[0] - iRadiusNeedle, ijk[1], ijk[2], 0
                                )
                                g3 = imgData.GetScalarComponentAsDouble(
                                    ijk[0], ijk[1] + iRadiusNeedle, ijk[2], 0
                                )
                                g4 = imgData.GetScalarComponentAsDouble(
                                    ijk[0], ijk[1] - iRadiusNeedle, ijk[2], 0
                                )
                                g5 = imgData.GetScalarComponentAsDouble(
                                    ijk[0] + iRadiusNeedleCorner,
                                    ijk[1] + iRadiusNeedleCorner,
                                    ijk[2],
                                    0,
                                )
                                g6 = imgData.GetScalarComponentAsDouble(
                                    ijk[0] - iRadiusNeedleCorner,
                                    ijk[1] - iRadiusNeedleCorner,
                                    ijk[2],
                                    0,
                                )
                                g7 = imgData.GetScalarComponentAsDouble(
                                    ijk[0] - iRadiusNeedleCorner,
                                    ijk[1] + iRadiusNeedleCorner,
                                    ijk[2],
                                    0,
                                )
                                g8 = imgData.GetScalarComponentAsDouble(
                                    ijk[0] + iRadiusNeedleCorner,
                                    ijk[1] - iRadiusNeedleCorner,
                                    ijk[2],
                                    0,
            ...
            ijkAPrev = ijkA
            rasAPrev = np.array(self.ijk2ras(ijkAPrev))
            ...
            if 1:  # <o>
                ...
                if ijkCBestPrev:
                    ...
                    if rasCBestPrev2CBest.any():
                        rasCBestPrev2CBestPrev = rasCBestPrev2CBest
                        rasCBestPrev2CBestPrev1 = rasCBestPrev2CBestPrev / norm(
                            rasCBestPrev2CBestPrev
                        )
                    rasCBestPrev2CBest = rasCBest - rasCBestPrev
                    rasCBestPrev2CBest1 = rasCBestPrev2CBest / norm(rasCBestPrev2CBest)
                rasA = np.array(self.ijk2ras(ijkA))
                if rasA2CBest.any():
                    rasA2CBestPrev = rasA2CBest
                    rasA2CBestPrev1 = rasA2CBestPrev / norm(rasA2CBestPrev)
                rasA2CBest = rasCBest - rasA
                rasA2CBest1 = rasA2CBest / norm(rasA2CBest)
                ...
                if (
                    ...
                    if (
                        ...
                        if rasB2CBest.any():
                            ...
                            rasB2CBest1 = np.array([0, 0, 0])
                        rasA = rasB + rasB2CBest1 * min(
                            fMaxDistB2CBest_mm, fDistB2CBest_mm / 2.0
                        )
                    ...
                    if iStep == 0 - abs(iStart):
                        breakbox("/!!\ not using CBest in first it.??")  # <<<
                    ...
                    if iStep <= -1:  # prepare init. of model
                        ...
                        if iStep == -1:
                            ...
                            ijkA = ijkA0
                            rasA = self.ijk2ras(ijkA)
                        elif iStep <= -2:
                            rasA2CBestInit.append(rasA2CBest)  # 1st init segment
                        continue  # rof iStep, skip rest of loop using this helper segments
                    ijkA = ijkCBest
                    rasA = self.ijk2ras(ijkA)
                    rasNeedlePlaneNormal = rasDeflectionDirection = np.zeros(
                        3
                    )  # <o>force recalc. of deflection direction
                    print("using_ijkC= ", ijkCBest)
                    print("best_fEstimator_value= ", fTotal)
                else:
                    breakbox("/!!| uncaught ijkCBest==ijkAPrev")
            ...
            if iStep >= 0:
                ...
                if ijkA[2] <= iAxialSegmentationLimit and ijkA != ijkA0:
                    ...
                    break
        # rof iStep
        for i in range(len(lvControlPointsRAS)):
            self.controlPoints.append(lvControlPointsRAS[i])
        if not bAutoStopTip:
            if bDrawNeedle:
                self.addNeedleToScene(
                    lvControlPointsRAS,
        ...
        if bUp:
            return lvControlPointsIJK[-1]  # return last control point as tip estimate
    ...
    def needleDetectionUPThread(
        ...
        # select algo version
        if widget.algoVersParameter.value == 0:
            return self.needleDetectionUPThreadCurrentDev(
                A,
                imageData,
                colorVar,
                spacing,
        ...
        if widget.algoVersParameter.value == 3:
            if widget.labelMapNode:
                return self.needleDetectionThread15_1(
                    A,
                    imageData,
                    widget.labelMapNode.GetImageData(),
                    widget.tempPointList,
                    colorVar,
                    spacing,
                    bUp=True,
    ...
    """
        ...
        for step in range(0, NbStepsNeedle + 1):
            ...
            P0 = A
            P1 = [P0[0] + K * Vx, P0[1] + K * Vy, P0[2] + K * Vz]
          ...
          """
                ...
                if minEstimator0 != None and minEstimator0 != 0:
                    RelMinEstimator = abs(
                        (minEstimator - minEstimator0) / float(minEstimator0)
                    )
            ...
            if bestPoint == [
                ...
                0,
                0,
            ]:  # if the initial point (center of the cone) is indeed the optimal ctrl point
                A = C0
            elif bestPoint != tip0:
                A = bestPoint
            ...
            if widget.drawFiducialPoints.isChecked():
                ...
                fiducial.SetName(
                    ...
                    + str(step)
                    + "."
                    + str(self.objectiveFunction(imageData, A, 3, spacing, 1))
                    + "_"
                    + str(colorVar)
                )
        ...
        if not tipOnly:
            self.addNeedleToScene(
                self.controlPoints, colorVar, "Detection", script, manualName=manualName
            )
        # print 'length:',tot
        ...
        for i in range(len(widget.obtuNeedlePt)):
            if widget.obtuNeedlePt[i][0][0] != 999:
                controlPointsUnsorted = [
                    val for val in widget.obtuNeedlePt[i] if val != [999, 999, 999]
                ]
    ...
    def drawValidationNeedles(self, nb=None):
        ...
        * For each sets of needle points, the function addNeedleToScene fits a Bezier curve and render it as a
        ...
        widget = slicer.modules.NeedleFinderWidget
        if nb:
            widget.editNeedleTxtBox.value = nb
        widget.initTableView()
        ...
        for i in range(len(tableValueCtrPt)):
            if not all(e == [999, 999, 999] for e in tableValueCtrPt[i]):
                ...
                colorVar = random.randrange(50, 100, 1)  # ??? /(100.)
                controlPointsUnsorted = [
                    val for val in tableValueCtrPt[i] if val != [999, 999, 999]
                ]
    ...
    """
        ...
        controlPointListSorted = (
            controlPoint  # self.sortTableReverse(controlPoint, (2, 1, 0))
        )

        # calculate the length of the list of ctr points
        lenghtTotal = 0
        for i in range(len(controlPoint) - 1):
            lenghtTotal += self.distanceTwoPoints(
                controlPointListSorted[i + 1], controlPointListSorted[i]
            )
    ...
    """
        if lenghtTotal < realNeedleLength and extendNeedle:
            lastPoint = [
                controlPointListSorted[-1][0],
                controlPointListSorted[-1][1],
                controlPointListSorted[-1][2] - (realNeedleLength - lenghtTotal),
            ]
        ...
        for i in range(numberOfInputPoints):
            x, y, z = (
                controlPointListSorted[i][0],
                controlPointListSorted[i][1],
                controlPointListSorted[i][2],
            )
    ...
    """
        ...
        if endMarker:
            ...
            fiducial.SetLocked(True)
            displayNode = fiducial.GetDisplayNode()
            displayNode.SetGlyphScale(0.5)  # "arrow head"
            displayNode.SetColor(0, 0, 0)
            textNode = fiducial.GetAnnotationTextDisplayNode()
            textNode.SetTextScale(1)
            textNode.SetColor(0, 0, 0)
        if bPause:
            breakbox("Inspect this situation in the viewer!")
    ...
    def deleteTempModels(self):
        ...
        while slicer.util.getNodes(".*") != {}:
            nodes = slicer.util.getNodes(".*")
            for key, value in zip(list(nodes.keys()), list(nodes.values())):
                if key != ".temp":
    ...
    def deleteLastNeedle(self):
        ...
        if widget.deleteNeedleButton.isEnabled() and self.lastNeedleNames:
            ...
            ID = (
                name.lstrip("auto-seg_")
                .lstrip("manual-seg_")
                .lstrip("obturator-seg_")
                .lstrip("0123456789")
                .lstrip("-ID-vtkMRMLModelNode")
                .rstrip("manual-seg_")
                .rstrip("0123456789")
            )
    ...
    def newInsertionNeedleSet(self):
        ...
        if widget.newInsertionButton:
            dialog = qt.QDialog()
            messageBox = qt.QMessageBox.information(
                dialog, "Information", "You are creating a new set of needles"
            )
            self.round += 1
            widget.newInsertionButton.setText(
                "Start a new set of needles - Round " + str(self.round + 1) + "?"
            )
            widget.deleteNeedleButton.setText(
                "Delete Needles from round " + str(self.round)
            )
    ...
    def resetNeedleDetection(self, script=False):
        ...
        if ret == qt.QMessageBox.Ok or script == True:
            ...
            if widget.newInsertionButton:
                widget.newInsertionButton.setText(
                    "Start a new set of needles (" + str(self.round + 1) + ") ?"
                )
                widget.deleteNeedleButton.setText(
                    "Delete Needles from set (" + str(self.round) + ")"
                )
            # reset report table
            widget.table = None
            widget.row = 0
            ...
            if not script:
                ...
                widget.model = None
                widget.view.reset()
                slicer.modules.NeedleFinderWidget.analysisGroupBoxLayout.removeWidget(
                    widget.view
                )
                del widget.view
                widget.view = None
    ...
    def changeValue(self):
        ...
        self.lockControlPoints(widget.editNeedleTxtBox.value)
        self.unlockControlPoints(widget.editNeedleTxtBox.value)
        widget.drawValidationNeedlesButton.text = "Render Manual Needle  " + str(
            widget.editNeedleTxtBox.value
        )
    ...
    def filterWithSITK(self):
        ...
        del backgroundImage
        sitk.WriteImage(
            filterImage, sitkUtils.GetSlicerITKReadWriteAddress(backgroundNodeName)
        )
    ...
    def parSearch(self, mode=False):
        ...
        if 0:
            path[
                24
            ] = "/Users/guillaume/Dropbox/AMIGO Gyn Data NRRD/Case 24 NRRD/Manual/2013-02-25-Scene-without-CtrPt.mrml"
            path[
                29
            ] = "/Users/guillaume/Dropbox/AMIGO Gyn Data NRRD/Case 29 NRRD/Manual/2013-02-26-Scene-without-CtrPts.mrml"
            path[
                30
            ] = "/Users/guillaume/Dropbox/AMIGO Gyn Data NRRD/Case 30 NRRD/Manual/2013-02-26-Scene-without-CtrPt.mrml"
            path[
                31
            ] = "/Users/guillaume/Dropbox/AMIGO Gyn Data NRRD/Case 31 NRRD/Manual/2013-02-27-Scene-without-CtrPts.mrml"
            path[
                34
            ] = "/Users/guillaume/Dropbox/AMIGO Gyn Data NRRD/Case 34 NRRD/Manual/2013-02-27-Scene-without-CtrPts.mrml"
            path[
                35
            ] = "/Users/guillaume/Dropbox/AMIGO Gyn Data NRRD/Case 35 NRRD/Manual/2013-02-27-Scene-without-CtrPts.mrml"
            path[
                37
            ] = "/Users/guillaume/Dropbox/AMIGO Gyn Data NRRD/Case 37 NRRD/Manual/2013-02-27-Scene-without-CtrPts.mrml"
            path[
                38
            ] = "/Users/guillaume/Dropbox/AMIGO Gyn Data NRRD/Case 38 NRRD/Manual/2013-02-27-Scene-without-CtrPts.mrml"
            path[
                40
            ] = "/Users/guillaume/Dropbox/AMIGO Gyn Data NRRD/Case 40 NRRD/Manual/2013-02-27-Scene-without-CtrPts.mrml"
        ...
        if 0:
            path[
                33
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  033/NRRD/Auto-Eval-LB/2013-02-27-Scene.mrml"
        if 0:
            path[
                8
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  008/NRRD/Auto-Eval-LB/2013-05-07-Scene.mrml"
            path[
                12
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  012/NRRD/Auto-Eval-LB/2013-04-22-Scene.mrml"
            path[
                16
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  016/NRRD/Auto-Eval-LB/2013-04-21-Scene.mrml"
            path[
                21
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  021/NRRD/Auto-Eval-LB/2013-04-21-Scene.mrml"
            path[
                22
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  022/NRRD/Auto-Eval-LB/2013-04-21-Scene.mrml"
            path[
                25
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  025/NRRD/Auto-Eval-LB/2013-04-21-Scene.mrml"
            path[
                26
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  026/NRRD/Auto-Eval-LB/2013-04-17-Scene.mrml"
            path[
                27
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  027/NRRD/Auto-Eval-LB/2013-04-17-Scene.mrml"
        # stripped MICCAI13 cases (just manual seg. by LB/AM)
        if 1:
            path[
                24
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  024/NRRD/Auto-Eval-LB/2013-02-28-Scene.mrml"
            path[
                28
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  028/NRRD/Auto-Eval-LB/2013-02-28-Scene.mrml"
            path[
                29
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  029/NRRD/Auto-Eval-LB/2013-02-26-Scene.mrml"
            path[
                30
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  030/NRRD/Auto-Eval-LB/2013-02-26-Scene.mrml"
            path[
                31
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  031/NRRD/Auto-Eval-LB/2013-02-27-Scene.mrml"
            path[
                33
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  033/NRRD/Auto-Eval-LB/2013-02-27-Scene.mrml"
            path[
                34
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  034/NRRD/Auto-Eval-LB/2013-02-27-Scene.mrml"
            path[
                37
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  037/NRRD/Manual Alireza/2013-02-27-Scene.mrml"
            path[
                38
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  038/NRRD/Manual Alireza/2013-02-27-Scene.mrml"
            path[
                40
            ] = "/home/mastmeyer/Dropbox/GYN Cases/Case  040/NRRD/Manual Alireza/2013-02-27-Scene.mrml"
        ...
        try:
            shutil.copyfile(
                "/home/amast/WualaDrive/mastmeyer/Homes/NeedleFinder/NeedleFinder/NeedleFinder.py",
                dir + "/NeedleFinder_ref.py",
            )
        except:
            breakbox("/!\ reference source NeedleFinder.py not found!")
        if mode == 0:
                ...
                for id in range(100):  # <o> range(100)
                    if path[id]:
                        ...
                        self.writeTableHeader(
                            dir + "/User-" + str(user) + "_AP-" + str(id) + ".csv", 1
                        )
                        ...
                        results, outliers = l.evaluate(
                            script=True
                        )  # calculate HD distances
                        for result in results:
                            result[0:0] = [user, id]
                        l.exportEvaluation(
                            results,
                            dir + "/User-" + str(user) + "_AP-" + str(id) + ".csv",
                        )
            ...
            for id in range(100):
                if path[id]:
                    ...
                    for i in range(1, 10000):
                        ...
                        w.sigmaValue.setValue(
                            np.random.randint(1, 40)
                        )  # change parameter sigma
    ...
    def addNeedleToTable(self, ID, label=None, needleType=None):
        ...
        color = qt.QColor()
        color.setRgb(
            self.color255[ref][0], self.color255[ref][1], self.color255[ref][2]
        )
        ...
        if needleType == "Validation":
            ID = int(
                slicer.util.getNode("manual-seg_" + str(ID))
                .GetID()
                .strip("vtkMRMLModelNode")
            )
        ...
        reformatButton.connect(
            "clicked()", lambda who=ID: self.reformatSagittalView4Needle(who)
        )
    ...
    def addPointToTable(
        ...
        color = qt.QColor()
        color.setRgb(
            self.color255[needleNumber][0],
            self.color255[needleNumber][1],
            self.color255[needleNumber][2],
        )
        item = qt.QStandardItem()
        item.setData(color, 1)
        ...
        if needleType == "Validation":
            ID = int(
                slicer.util.getNode("." + str(ID)).GetID().strip("vtkMRMLModelNode")
            )
        listArgs = [ID, needleNumber, pointNumber]
        displayButton.connect(
            "clicked()", lambda who=listArgs: self.removeNeedleShaftEvalMarker(who)
        )
  ...
  """
            ...
            if node.GetAttribute("type") == type:
                ...
                if (
                    ...
                    if not widget.autoStopTip.isChecked():
                        ...
                        polydata.GetPoint(
                            int(polydata.GetNumberOfPoints() - 1 - offset), pbis
                        )
                        if center:
                            polydata.GetPoint(0 + offset + 25, pOpp)
                        ...
                        polydata.GetPoint(
                            int(polydata.GetNumberOfPoints() / 2) + offset, p
                        )  # CONST
                        if center:
                            polydata.GetPoint(
                                int(polydata.GetNumberOfPoints() / 2) + offset + 25,
                                pOpp,
                            )
                            p = (np.array(p) + np.array(pOpp)) / 2
    ...
    def startValidation(self, script=False, offset=0, needleNr=None):
        ...
        * Calls evaluate() to compute the Hausdorff's distance between every pair of needles
        ...
        self.t0 = time.clock()
        print("spacing: ", spacing)
        self.needleDetectionThread(
            tips, imageData, spacing=spacing, script=script, names=names
        )  # <<< Ruibin
    ...
    def evaluate(self, script=False, needleNr=None):
        ...
        self.valuesExperience = [
            widget.radiusNeedleParameter.value,
        ...
        for i in range(len(result)):
            ...
            val = self.hausdorffDistance(result[i][1], result[i][2])
            try:
                needleNrFromFilename = int(result[i][3].strip("manual-seg_"))
            except:
                needleNrFromFilename = -1
    ...
    def distTip(self, id1, id2):
        ...
        for i in range(100):
            ...
            if p2bis[2] > p2[2]:
                p2 = p2bis
            tipDistance.append(
                ((p2[0] - p[0]) ** 2 + (p2[1] - p[1]) ** 2 + (p2[2] - p[2]) ** 2) ** 0.5
            )
            ...
            if "auto-seg" in node.GetName():
                ...
                if polydata is not None and polydata.GetNumberOfPoints() > 100:
                    for nthNode2 in range(nbNode):
                        node2 = slicer.mrmlScene.GetNthNodeByClass(
                            nthNode2, "vtkMRMLModelNode"
                        )
                        if "manual-seg" in node2.GetName():
                            ...
                            if (
                                ...
                                tipDistance = self.distTip(
                                    ...
                                    int(node2.GetID().strip("vtkMRMLModelNode")),
                                )
                                dist.append(
                                    [tipDistance, node2.GetID(), node2.GetName()]
                                )
                                # print tipDistance
                    if dist != []:
    ...
    def saveParameters(self, filePath):
        ...
        config.set(
            "BooleanSection", "autoCorrectTip", widget.autoCorrectTip.isChecked()
        )
        config.set(
            "BooleanSection", "invertedContrast", widget.invertedContrast.isChecked()
        )
        config.set("BooleanSection", "gradient", widget.gradient.isChecked())
        config.set(
            "BooleanSection",
            "filterControlPoints",
            widget.filterControlPoints.isChecked(),
        )
        config.set(
            "BooleanSection",
            "drawFiducialPoints",
            widget.drawFiducialPoints.isChecked(),
        )
        ...
        config.set(
            "BooleanSection",
            "gaussianAttenuationButton",
            widget.gaussianAttenuationButton.isChecked(),
        )
        ...
        config.set(
            "IntegerSection", "gradientPonderation", widget.gradientPonderation.value
        )
        config.set("IntegerSection", "exponent", widget.exponent.value)
        config.set("IntegerSection", "distanceMax", widget.radiusMax.value)
        config.set(
            "IntegerSection", "nbRotatingIterations", widget.nbRotatingIterations.value
        )
        config.set(
            "IntegerSection",
            "numberOfPointsPerNeedle",
        ...
        config.set(
            "IntegerSection", "algoVersParameter", widget.algoVersParameter.value
        )
    ...
    def loadParameters(self, filePath):
        ...
        gaussianAttenuationButton = config.getboolean(
            "BooleanSection", "gaussianAttenuationButton"
        )
        ...
        try:
            radiusMax = config.getint(
                "IntegerSection", "distanceMax"
            )  # try deprecated parameter name (old parameter files)
        except:
            radiusMax = config.getint("IntegerSection", "radiusMax")
        nbRotatingIterations = config.getint("IntegerSection", "nbRotatingIterations")
        numberOfPointsPerNeedle = config.getint(
            "IntegerSection", "numberOfPointsPerNeedle"
        )
    ...
    def autoregistration(self):
        ...
        #####################################################################################
        print("cropVolume")
        cropVolumeNode = slicer.mrmlScene.CreateNodeByClass(
        ...
        roiSegmentation = vl.CreateAndAddLabelVolume(
            slicer.mrmlScene, roiVolume, "marker_segmentation"
        )
        ...
        for i in range(lo, hi + 1):
            ...
            thresholder.SetOutputScalarType(
                roiSegmentation.GetImageData().GetScalarType()
            )
            thresholder.Update()
    ...
    def loadTemplate(self):
        ...
        if not bool(slicer.util.getNode("Template")):
            pathToScene = slicer.modules.igyne.path.replace(
                "iGyne/iGyne.py", "iGyne/Resources/Template/4points/Template.mrml"
            )
    ...
    def getCenterOfMass(self, volumeNode, step):
        ...
        if volumeNode.GetLabelMap() == 0:
            print(
                (
                    "Warning: input volume is not labelmap: '"
                    + volumeNode.GetName()
                    + "'"
                )
            )
        ...
        for z in range(volume.GetExtent()[4], volume.GetExtent()[5] + 1):
            for y in range(volume.GetExtent()[2], volume.GetExtent()[3] + 1, step):
                for x in range(volume.GetExtent()[0], volume.GetExtent()[1] + 1, step):
    ...
    def firstRegistration(self):
        ...
        for nodeIndex in range(sliceNodeCount):
            sliceNode = slicer.mrmlScene.GetNthNodeByClass(
                nodeIndex, "vtkMRMLAnnotationHierarchyNode"
            )
        ...
        RMS = ""
        parameters = {}
        parameters["fixedLandmarks"] = slicer.util.getNode(
            "Fiducial List_fixed"
        ).GetID()
        parameters["movingLandmarks"] = slicer.util.getNode(
            "Fiducial List_moving"
        ).GetID()
        ...
        __cliObserverTag = __cliNode.AddObserver(
    ...
    def controlPointsEstimation(self, n=None, N=11):
        nodes = slicer.util.getNodes("manual-seg_*")
        for key, node in zip(list(nodes.keys()), list(nodes.values())):
            ...
            for previousPt in list(previousPts.values()):
                ...
                if lp[2] > pt[2]:
                    candidates.append(
                        [
                            name,
                            self.angle(
                                self.getOrientationVect(pt, lp),
                                self.getNeedleOrientation(nb),
                            ),
                        ]
                    )
    ...
    def findLowerNeedles(self, pt):
        ...
        for node in list(nodes.values()):
            ...
            if nb in validNeedles:
                ...
                if hp[2] < pt[2]:
                    theta = self.angle(
                        self.getNeedleOrientation(nb), -self.getOrientationVect(pt, hp)
                    )
    ...
    def assignNeedle(self, pt):
        ...
        if cLow:
            lowest = self.sortTable(cLow, (1, 0))[0]
        if cHigh:
            highest = self.sortTable(cHigh, (1, 0))[0]
        if cClose:
            closest = self.sortTable(cClose, (1, 0))[0]
        ...
        if result:
            result = int(result.strip("manual-seg_"))
        if not result:
            candidates = self.findMatchingCTLPoint(mo, pt)
            if candidates:
                result = self.sortTable(candidates, (1, 0))[0][0]
        if not result:
            print("no good candidate to assign to -> new point")
        return result

    def findNeedles(self, all=None):
        needleNbs = []
        needles = slicer.util.getNodes("manual-seg_*")
        for needle in list(needles.values()):
            name = needle.GetName()
    ...
    def findIsolatedCTLPoints(self):
        ...
        for nb in needleNbs:
            points = slicer.util.getNodes(".%d-*" % nb)
            if len(points) < 2:
                isolatedPts.append(list(points.values())[0])
