...
class NeedleFinderWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    # TODO remove references to EditUtils
    ...
    def laurenFeature(self):
        ...
        """
        vnJPG = slicer.util.getNode(
            "Case *"
        )  # the naming convention for the ground truth JPG files: "Case XXX.jpg"
        if vnJPG:
...
class NeedleFinderLogic:
    ...
    def array2(self):
        """
        Used if needle tips input is given through a labelmap.
        Extract the coordinates of each label (after IslandEffect)
        """
        u.profbox(u.whoami())
        # research
                    ...
                    for i in range(children.GetNumberOfItems()):
                        ...
                        if oneToOneHierarchyNode:
                            try:
                                ...
                                u.logprint(msg="pb removing onetoone")
                        if pointDisplayNode:
                            try:
                                scene.RemoveNode(pointDisplayNode)
                            except:
                                u.logprint(msg="pb removing pointDisplayNode")
                        if textDisplayNode:
                            try:
                                scene.RemoveNode(textDisplayNode)
                            except:
                                u.logprint(msg="pb removing textDisplayNode")
                        if storageNode:
                            try:
                                scene.RemoveNode(storageNode)
                            except:
                                u.logprint(msg="pb removing storageNode")
                        if annotNode:
                            try:
                                scene.RemoveNode(annotNode)
                            except:
                                u.logprint(msg="pb removing annotNode")
    ...
    def findAxialSegmentationLimitFromMarker(self, bForceFallback=False):
        ...
        for node in list(nodes.values()):
            ...
            if found:
                print(r"/!\ there should be only one axial limit marker!")
            found = True
        if not found or bForceFallback:
            print(r"/!\ z-limit marker in scene required! --> fallback")
            bases, names = self.returnBasesFromNeedleModels()
            bases2 = np.array(bases)[:, 2]
    ...
    def needleDetectionAPI(
        ...
        if not widget.algoVersParameter.value in range(0, 6):
            u.msgbox(
                r"/!\ needleDetectionThread %d not defined"
    ...
    def needleDetectionThread_RM(
        ...
        t0 = time.clock()
        bDraw = True
        # u.msgbox("Detour: \!/ This site is under heavy construction. /!\ ")
        widget = slicer.modules.NeedleFinderWidget
        ...
        for NeedleIndex in range(NumberOfNeedles):
            ...
            if widget.algoVersParameter.value == 1:
                ...
                u.msgbox(
                    r"/!\ single needleDetectionThread %d not defined"
                    % widget.algoVersParameter.value
                )
    ...
    def needleDetectionThreadCurrentDev(
        ...
        pixelValue = np.zeros(shape=(dims[0], dims[1], dims[2]))
    ...
    """
        ...
        for iAbsStep, iStep in enumerate(range(iStart, nStepsNeedle)):
            ...
            if ijkCBest == [np.nan, np.nan, np.nan]:
                u.breakbox(r"/!!\ no ijkCBest found")
            ...
            if 1:  # <o>
                ...
                print("ijkCBest= ", ijkCBest, "# line: ", u.lineno())
                ...
                if (
                    ...
                    if iStep == 0 - abs(iStart):
                        u.breakbox(r"/!!\ not using CBest in first it.??")  # <<<
                    ...
                    u.breakbox("/!!| uncaught ijkCBest==ijkAPrev")
    ...
    def needleDetectionUPThread(
        ...
        if widget.algoVersParameter.value == 1:
            u.msgbox(r"/!\ needleDetectionUPThread N/A for algoVersion 1")
        if widget.algoVersParameter.value == 2:
            u.msgbox(r"/!\ needleDetectionUPThread N/A for algoVersion 2")
    ...
    """
        ...
        if endMarker:
            ...
            textNode.SetColor(0, 0, 0)
        if bPause:
            u.breakbox("Inspect this situation in the viewer!")
    ...
    def parSearch(self, mode=False):
        ...
        try:
            ...
            u.breakbox(r"/!\ reference source NeedleFinder.py not found!")
        if mode == 0:
            ...
            for user in range(nUsers):
                ...
                for id in range(100):  # <o> range(100)
                    if path[id]:
                        ...
                        # pause()
            u.msgbox("parSearch mode 0 done, results in " + dir)
        elif mode == 1:
            id = "Current"
            ...
            for i in range(3, 12):
                ...
                # pause()
            u.msgbox("parSearch mode 1 done, results in " + dir)
        elif mode == 2:
            ...
            for id in range(100):
                if path[id]:
                    ...
                    for i in range(1, 10000):
                        ...
                        # start = time.time()
                        # pause()
                u.msgbox("parSearch mode 2 done, results in " + dir)
            # rof id
        # file mode 2
    ...
    def evaluate(self, script=False, needleNr=None):
        ...
        if script == False:
            return np.array(HD).astype(np.double)
        else:
            return HD, outliers
    ...
    def addManualTip(self, A):
        ...
        """
        self.createMarkup(name="tip", position=A, color=[0, 1, 0], visible=True)
    ...
    def getAndSortFiducialPoints(self, center):
        ...
        for k in range(4):
            fiducial = self.createMarkup(
                name=str(k), position=markerCenters[sorted2[k]]
            )
    ...
    def getCenterOfMass(self, volumeNode, step):
        ...
        if volumeNode.GetLabelMap() == 0:
            print(
                "Warning: input volume is not labelmap: '" + volumeNode.GetName() + "'"
            )
        #
